...
interface SecondhandAttributes {
  id: number;
  title: string;
  price: number;
  category: string;
...
async function initDatabase(): Promise<boolean> {
  ...
  try {
    // 尝试连接到MySQL默认数据库
    const initSequelize = new Sequelize('mysql', dbConfig.username, dbConfig.password, {
      host: dbConfig.host,
      port: dbConfig.port,
      dialect: 'mysql',
      logging: false,
    ...
    global.Vehicle = global.sequelize.define('Vehicle', {
      ...
      mileage: DataTypes.FLOAT, // 行驶里程（万公里）
      region: DataTypes.STRING, // 所属地区
      validity: DataTypes.STRING, // 有效期
      condition: DataTypes.STRING, // 新旧程度
      source: DataTypes.STRING, // 来源（个人/商家）
      description: DataTypes.TEXT,
      image: DataTypes.STRING,
      images: DataTypes.TEXT('long'), // 多张图片
      contactPerson: DataTypes.STRING, // 联系人
      contactPhone: DataTypes.STRING, // 联系电话
      contactQQ: DataTypes.STRING, // 联系QQ
      userId: DataTypes.INTEGER,
      status: {
        type: DataTypes.STRING,
        defaultValue: 'pending' // pending, approved, rejected
    ...
    global.User = global.sequelize.define('User', {
      ...
      password: DataTypes.STRING,
      userType: DataTypes.STRING, // 'personal' 或 'organization'
      // 个人用户字段
      ...
      organizationDesc: DataTypes.TEXT,
      contactPerson: DataTypes.STRING,
      qq: DataTypes.STRING,
      address: DataTypes.STRING,
      // 金币系统
    ...
    global.WebsiteSettings = global.sequelize.define('WebsiteSettings', {
      ...
      type: DataTypes.STRING, // 'siteMechanism', 'contact', 'member', 'ad', 'about', 'siteInfo'
      title: DataTypes.STRING,
      content: DataTypes.TEXT,
      phone: DataTypes.STRING, // 联系电话
      email: DataTypes.STRING, // 邮箱
      address: DataTypes.STRING, // 地址
      companyName: DataTypes.STRING, // 公司名
      logoUrl: DataTypes.TEXT('long'), // Logo URL
      paymentQrCode: DataTypes.TEXT('long'), // 付款二维码URL
      icpNumber: DataTypes.STRING, // ICP备案号
...
app.put('/api/houses/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (req.body.area) house.area = req.body.area;
    if (req.body.type) house.type = req.body.type;
    if (req.body.layout) house.layout = req.body.layout;
    if (req.body.floor) house.floor = req.body.floor;
    if (req.body.decoration) house.decoration = req.body.decoration;
    if (req.body.orientation) house.orientation = req.body.orientation;
    if (req.body.location) house.location = req.body.location;
    if (req.body.facilities) house.facilities = req.body.facilities;
    if (req.body.contactPerson) house.contactPerson = req.body.contactPerson;
    if (req.body.contactPhone) house.contactPhone = req.body.contactPhone;
    if (req.body.description) house.description = req.body.description;
    if (req.body.status) house.status = req.body.status;
...
// 二手物品
app.get('/api/secondhand', async (req: Request, res: Response) => {
  try {
    const { limit, status, userId, page, pageSize, search, contactPhone } = req.query;
    const where: any = {};
    // 如果指定了userId，则不限制status，让用户可以看到自己发布的所有信息
...
app.get('/api/secondhand/:id', async (req: Request, res: Response) => {
  try {
...
app.post('/api/secondhand', upload.none(), async (req: Request, res: Response) => {
  try {
    // 处理图片数据
    let image = '';
    let images = '';
...
app.delete('/api/secondhand/:id', async (req: Request, res: Response) => {
  try {
...
app.put('/api/secondhand/:id', upload.none(), async (req: Request, res: Response) => {
  try {
    const item = await global.Secondhand.findByPk(req.params.id);
    ...
    if (req.body.contactPerson) item.contactPerson = req.body.contactPerson;
    if (req.body.contactPhone) item.contactPhone = req.body.contactPhone;
...
app.post('/api/vehicles', upload.none(), async (req: Request, res: Response) => {
  try {
    // 处理图片数据
    let image = '';
    let images = '';
...
app.put('/api/vehicles/:id', upload.none(), async (req: Request, res: Response) => {
  try {
    const vehicle = await global.Vehicle.findByPk(req.params.id);
    if (req.body.title) vehicle.title = req.body.title;
    ...
    if (req.body.region) vehicle.region = req.body.region;
    if (req.body.validity) vehicle.validity = req.body.validity;
    if (req.body.condition) vehicle.condition = req.body.condition;
    if (req.body.source) vehicle.source = req.body.source;
    if (req.body.description) vehicle.description = req.body.description;
    if (req.body.status) vehicle.status = req.body.status;
    if (req.body.admin === 'true') vehicle.status = 'approved'; // 管理员更新的内容直接通过审核
    if (req.file) vehicle.image = req.file.filename;
    if (req.body.images) vehicle.images = req.body.images;
...
app.post('/api/services', upload.none(), async (req: Request, res: Response) => {
  try {
    // 处理图片数据
    let image = '';
    let images = '';
...
app.put('/api/services/:id', upload.none(), async (req: Request, res: Response) => {
  try {
    const service = await global.Service.findByPk(req.params.id);
    ...
    if (req.body.contact) service.contact = req.body.contact;
    if (req.body.contactPerson) service.contactPerson = req.body.contactPerson;
    if (req.body.contactPhone) service.contactPhone = req.body.contactPhone;
    if (req.body.contactQQ) service.contactQQ = req.body.contactQQ;
    if (req.body.region) service.region = req.body.region;
    if (req.body.validity) service.validity = req.body.validity;
...
app.put('/api/users/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (req.body.organizationName) user.organizationName = req.body.organizationName;
    if (req.body.region) user.region = req.body.region;
    if (req.body.industry) user.industry = req.body.industry;
    if (req.body.organizationDesc) user.organizationDesc = req.body.organizationDesc;
    if (req.body.contactPerson) user.contactPerson = req.body.contactPerson;
...
// 审核管理 - 获取所有待审核的内容
app.get('/api/moderation/pending', async (req: Request, res: Response) => {
  try {
...
// 审核管理 - 批准内容
app.put('/api/moderation/approve/:type/:id', async (req: Request, res: Response) => {
  try {
...
// 审核管理 - 拒绝内容
app.put('/api/moderation/reject/:type/:id', async (req: Request, res: Response) => {
  try {
    const { type, id } = req.params;
    let model: any;
...
// 用户注册 - 个人会员
app.post('/api/register/personal', async (req: Request, res: Response) => {
  try {
    // 检查用户名是否已存在
...
// 用户注册 - 机构会员
app.post('/api/register/organization', async (req: Request, res: Response) => {
  try {
    // 检查用户名是否已存在
    const existingUser = await global.User.findOne({ where: { username: req.body.username } });
    if (existingUser) {
...
// 用户登录
app.post('/api/login/user', async (req: Request, res: Response) => {
  try {
    // 查找用户
...
app.get('/api/website-settings/:type', async (req: Request, res: Response) => {
  try {
...
app.post('/api/website-settings', async (req: Request, res: Response) => {
  try {
    // 检查是否已存在该类型的设置
...
app.put('/api/website-settings/:id', async (req: Request, res: Response) => {
  try {
    const setting = await global.WebsiteSettings.findByPk(req.params.id);
    if (req.body.title) setting.title = req.body.title;
    if (req.body.content) setting.content = req.body.content;
    ...
    if (req.body.companyName) setting.companyName = req.body.companyName;
    if (req.body.logoUrl) setting.logoUrl = req.body.logoUrl;
    if (req.body.paymentQrCode) setting.paymentQrCode = req.body.paymentQrCode;
    if (req.body.icpNumber) setting.icpNumber = req.body.icpNumber;
    if (req.body.icpUrl) setting.icpUrl = req.body.icpUrl;
...
app.delete('/api/website-settings/:id', async (req: Request, res: Response) => {
  try {
