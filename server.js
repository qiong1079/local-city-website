...
const fs = require('fs'); // 新增：用于创建上传文件夹

const app = express();
const port = process.env.PORT || 3001;

// ========== 修复点1：创建multer上传文件夹（避免路径不存在报错） ==========
...
// 先连接到MySQL默认数据库创建localcity数据库
const initSequelize = new Sequelize('mysql', 'root', '123456', {
  host: '127.0.0.1',
  dialect: 'mysql',
  logging: console.log
});
...
app.get('/api/houses', async (req, res) => {
  try {
    ...
    // 搜索功能（按联系电话）
    if (contactPhone) {
      where.contactPhone = { [Sequelize.Op.like]: `%${contactPhone}%` };
    }
...
// ========== 修复点3：添加全局错误监听，避免崩溃 ==========
app.on('error', (err) => {
  console.error('Express服务器错误：', err);
});
